    ...
    try:
        ...
        if main:  # check if character ability was the main ability
            if character.name == ClassCharacterName.assassin.value:  # check if the character is the assassin
                log = character_helpers.use_assassin_ability(game_uuid, name_character, player.name)  # use assassin's ability and update log
                ...
                log = character_helpers.use_thief_ability(game_uuid, name_character, player.name)  # use thief's ability and update log

                if isinstance(log, tuple):  # check if error message
                    return log  # in this case it will contain the error message

            elif character.name == ClassCharacterName.magician.value:  # check if the character is the magician
                log = character_helpers.use_magician_ability(game_uuid, player_uuid, other_player_uuid, player.name, name_districts)  # use magician's ability and update log

                if isinstance(log, tuple):  # check if error message
                    return log  # in this case it will contain the error message

            elif character.name == ClassCharacterName.warlord.value:  # check if the character is the warlord
                log = character_helpers.use_warlord_ability(game_uuid, player, other_player_uuid, name_districts)  # use warlord's ability and update log
            ...
            log = character_helpers.use_secondary_ability(player, character)  # use secondary ability and update log
