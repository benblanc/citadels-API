...
def get_removed_characters(game_uuid, sort_order, order_by, limit, offset):
    try:
        ...
        if default_order_by == 'name':
            sort = removed_characters_db.name
        ...
        if default_limit == 0:
            characters = removed_characters_db.query.filter_by(game_uuid=game_uuid).order_by(sort).offset(default_offset).all()
        else:
            characters = removed_characters_db.query.filter_by(game_uuid=game_uuid).order_by(sort).limit(default_limit).offset(default_offset).all()
