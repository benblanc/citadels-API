...
def get_buildings(game_uuid, player_uuid, sort_order, order_by, limit, offset):
    try:
        ...
        if default_limit == 0:
            buildings = buildings_db.query.filter_by(player_uuid=player_uuid).order_by(sort).offset(default_offset).all()
        else:
            buildings = buildings_db.query.filter_by(player_uuid=player_uuid).order_by(sort).limit(default_limit).offset(default_offset).all()
